...
API 调用按 fast/standard/deep 档位收紧输出与重试预算；provider 思考参数仅在显式配置时透传。
...
REASONING_POLICIES = {
    "fast": {"max_tokens": 1800, "retries": 1},
    "standard": {"max_tokens": 900, "retries": 1},
    "deep": {"max_tokens": 1600, "retries": 2},
    "xdeep": {"max_tokens": 32768, "retries": 2},
}
OPERATION_REASONING_PROFILES = {
    "ingest_api_keywords": "fast",
    "ingest_api_repair": "fast",
    "ingest_api_claims": "standard",
    "ingest_wiki_write": "xdeep",
    "ingest_wiki_repair": "xdeep",
    "ingest_semantic_fill": "standard",
    "ingest_semantic_repair": "standard",
    # 命题拆分是短 JSON 抽取，不需要长链推理；fast 同时限制 provider effort、
    # 输出预算和自动重试，避免 reasoning token 吞掉摄入关键路径。
...
def reasoning_profile(config: dict[str, str], operation: str, requested: str | None = None) -> str:
    """解析操作档位；显式调用参数 > 操作配置 > 默认配置 > 内置映射。"""
    operation_key = "LLM_REASONING_" + re.sub(r"[^A-Z0-9]+", "_", operation.upper())
    profile = requested or config.get(operation_key) or config.get("LLM_REASONING_DEFAULT") or OPERATION_REASONING_PROFILES.get(operation, "standard")
...
def call_json(prompt: str, schema_check, *, system: str = "你是受程序约束的知识库组件，只输出要求的 JSON。", max_tokens: int = 800, retries: int = 1, operation: str = "query", reasoning: str | None = None, messages: list[dict] | None = None) -> dict:
    config = load_env()
    profile_name = reasoning_profile(config, operation, reasoning)
    policy = REASONING_POLICIES[profile_name]
    effective_max_tokens = min(max_tokens, policy["max_tokens"])
    effective_retries = min(retries, policy["retries"])
    ...
    if mode == "agent":
        ...
        _log_event(operation, handoff, prompt)
    ...
    if not all(profiles[0][key] for key in ("base", "key", "model")):
        ...
        _log_event(operation, _cfg_err, prompt)
        return _cfg_err
    last = None
    history = []
    for profile_index, profile in enumerate(profiles):
        attempt_max_tokens = effective_max_tokens
        attempt_reasoning = dict(reasoning_options)
        reasoning_strikes = 0
        for attempt in range(effective_retries + 1):
            ...
            try:
                ...
                last = {"ok": error is None, "status": "ok" if error is None else "validation_error", "mode": "api", "model": profile["model"], "profile": profile["name"], "reasoning_profile": profile_name, "max_tokens": used_max_tokens, "next_max_tokens": attempt_max_tokens if attempt < effective_retries else None, "attempt": attempt + 1, "text": text, "parsed": obj, "error": error, "latency_sec": round(time.time() - started, 2), "finish_reason": finish_reason, "usage": usage, "reasoning_tokens": reasoning_tokens, "reasoning_exhausted": reasoning_exhausted}
            except Exception as exc:
                retryable, err_label = _classify_http_error(exc)
                fast_fail = not retryable
                last = {"ok": False, "status": "api_error", "model": profile["model"], "profile": profile["name"], "reasoning_profile": profile_name, "max_tokens": attempt_max_tokens, "attempt": attempt + 1, "error": err_label, "latency_sec": round(time.time() - started, 2), "usage": {}}
            history.append({key: last.get(key) for key in ("model", "profile", "reasoning_profile", "max_tokens", "status", "attempt", "latency_sec", "usage")})
            if last["ok"]:
                last["history"] = history
                last["fallback_used"] = profile_index > 0
                _log_event(operation, last, prompt, last.get("text", ""))
                return last
    ...
    _log_event(operation, _final, prompt, (last or {}).get("text", ""))
    return _final


def call_text(prompt: str, *, system: str = "你是受程序约束的知识库组件，只输出要求的文本。", max_tokens: int = 1600, retries: int = 1, operation: str = "query", reasoning: str | None = None, messages: list[dict] | None = None) -> dict:
    ...
    for profile_index, profile in enumerate(profiles):
        ...
        for attempt in range(effective_retries + 1):
            ...
            try:
                ...
                last = {"ok": error is None, "status": "ok" if error is None else "validation_error", "mode": "api", "model": profile["model"], "profile": profile["name"], "reasoning_profile": profile_name, "max_tokens": used_max_tokens, "next_max_tokens": attempt_max_tokens if attempt < effective_retries else None, "attempt": attempt + 1, "text": text, "error": error, "latency_sec": round(time.time() - started, 2), "finish_reason": finish_reason, "usage": usage, "reasoning_tokens": reasoning_tokens, "reasoning_exhausted": reasoning_exhausted}
            except Exception as exc:
                retryable, err_label = _classify_http_error(exc)
                fast_fail = not retryable
                last = {"ok": False, "status": "api_error", "model": profile["model"], "profile": profile["name"], "reasoning_profile": profile_name, "max_tokens": attempt_max_tokens, "attempt": attempt + 1, "error": err_label, "latency_sec": round(time.time() - started, 2), "usage": {}}
