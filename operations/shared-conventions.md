...
1. 效果导向(六维联合效果,非技术偏好)
2. 单一事实源(主知识一份,视图派生;v4:节点属性在 md、边在 graph.db 分离式)
3. 证据保持与可回溯(每层带 sources 链回溯 raw)
4. 语义-确定性分离(原子任务级:语义归 LLM,机械归程序)
5. Schema 契约(语义开放、接口封闭)
6. 闭环提交(LLM 提议→程序校验→定向修复→复验→派生提交)
7. 能验证则验证(程序可机械判断的不依赖 LLM 自检)
8. 能派生则派生(可稳定重建的不让 LLM 重复生成)
9. 失败显式化(不静默降级,错误可定位)
10. 最小充分读取(读最小成本信息,停止条件是完成度非固定层数)
11. 规模不变性(单次操作消耗不随库体量增长)
12. 局部即时 vs 全局周期(摄入查局部,全库健康周期扫)
13. 最小机制(不重复造轮子 + 奥卡姆)
