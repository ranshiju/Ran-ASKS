...
> **触发**:标准 Graph/Wiki 召回与 Raw 证据读取后，答案槽位仍无候选时，读取本文件选择定向恢复策略。
> **定位**:位于槽位缺口回检之后、开放式联想之前的经验化检索提示。
> **顺序**:结构化召回 → Wiki/Raw 取证 → 槽位缺口回检 → **读 RECOVERY** → 开放式联想 → 限定性回答。
...
- **策略**:用图关系查同实体的 `superseded_by` 版本链；或查 page-catalog 定位同标题不同日期页。各版本逐个声明为槽位
...
- **策略**:该 section 可能按时间分段。①读全该页 Content（非仅 Navigation 摘要）②沿 sources 下钻 Raw ③用图 relations/temporal 查时间谓词（发生于/导致/修订为）
