...
`recognize_image()` 是显式授权的 API-only adapter，直接调用图片 chat/completions，不使用 DSH 或语义生成控制循环。
Agent 默认识别由当前宿主完成，API 无授权则明确失败，不隐式回退。

支持单帧 PNG/JPEG/WebP/BMP/TIFF（大小写扩展名均可），最大 20 MiB、4000 万像素；
图片解码需要当前 Python 环境已有 Pillow。
多帧输入明确拒绝，避免丢失后续页。无字插图不伪造转写，另由 Agent 判断图像语义。
...
图片进入 Wiki/语义生成前须完成风险评估。Agent backend 在转写后交接 `image_review` 暂存任务；
API backend 缺少复核时停止在 preprocess，提供已复核 `--ocr-result` 后可 resume，不隐式切换 Agent。
复核与 OCR 分离；更换模型或生成回执不等于完成复核。

```bash
# 默认 Agent 模式：先返回原图→文字的 agent-task-v1，再 resume 原事务。
python3 .scripts/ingest_inbox.py --run --file inbox/image.jpg --subproject admin

# OCR 结果复用：不再次上传；同一回执可在其他任务复用。
python3 .scripts/ingest_document.py --file inbox/image.jpg --subproject admin \
  --ocr-result temp/image-ocr/example.json

# 显式授权这次图片识别调用 API，不改变 Wiki/语义 backend。
python3 .scripts/ingest_document.py --file inbox/image.jpg --subproject admin --allow-remote-ocr
```

统一入口 `ingest_inbox.py --run --file ...` 同样支持 `--ocr-result` 与 `--allow-remote-ocr`，
两者只允许单图片，不对整个 inbox 批量授予上传许可；Agent/API dispatch 原样透传。

Agent OCR 任务只有原图、暂存输出路径、转写协议、源哈希与 resume 命令。
源变化时拒绝消费旧产物；无效/缺失转写仍为 prepared，不误判为 completed。
API 摄入也必须显式授权远程 OCR，或提供已有回执，不以 `INGEST_BACKEND=api` 代替图片上传许可。
识别与语义 backend 独立：Agent 可显式调用 API OCR 工具，再由当前 Agent 完成 Wiki/语义任务。
...
纯照片或示意图若无可识别文字，当前 OCR 路径明确停止并交由 Agent 判断，不伪造转写，
也不自动把模型生成的图像描述冒充原图文字。

### 复核记录与风险闸门

用 `image-ocr-review-v1` JSON 记录 `source_sha256`、`text_sha256`、`reviewer_kind`（agent/human）、
...
`status`（verified/unresolved）、`note`。Agent 必须实际对照原图，不把自身复核声称为人工确认。
...
无未决项且存在核对项时为 agent-reviewed/human-reviewed，否则为 partial；这不证明来源真实，也不代表事项获批。
