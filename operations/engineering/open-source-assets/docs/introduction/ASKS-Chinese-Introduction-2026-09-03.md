...
Agent 模式准备好的摄入事务现在可以通过 transaction ID 跨宿主会话接续。宿主先用
`wg.py task inspect` 读取类型化任务、声明的输入输出与下一步动作，再在必需输出存在后
用 `wg.py task advance` 执行原事务声明的检查、恢复或提交。任务记录不复制来源正文，
暂存回执也不替代 Raw、Wiki 或 graph.db 中的事实与导航状态。

私有知识采用独立 Raw／Wiki 目录和 `private/graph.db`，私有摄入、查询、索引与回执均
保持物理隔离。它们复用来源寻址和确定性校验，但不会把私有节点并入公共域图。公开仓库
只包含通用隔离实现和合成测试，不包含用户的私有知识、数据库或本机状态。

原生演示文稿处理进一步拆分为确定性结构提取与绑定来源的视觉识读：结构层保存文字、
表格、对象和位置，视觉层核对页面呈现和非文本信息。可复用幻灯片组件保留来源 deck、
可编辑性、布局 profile 与回执，项目图稿通过受管的可编辑矢量路径生成；任何远程调用
仍需显式授权。环境配置还允许用 `LLM_API_PATH` 与 `EMBED_API_PATH` 分别指定 Chat 和
Embedding 路径，适配不同兼容服务而无需修改代码。
...
## 2026年9月23日工程更新补充（v0.7.0）

本次以兼容新增能力从 v0.6.0 升至 v0.7.0：建立统一的运行时功能注册表，并加入模板驱动简历和原生演示文稿制作等受管产物工作流；同时强化 PDF 提取、摄入恢复、工程影响分析和图校验。本次更新不改变既有论文实验、冻结数据产物或其复现边界，也不创建新标签或 Release。

### 功能与持续状态由统一注册表区分

ASKS 现在由规范运行时功能注册表统一描述任务入口、CLI 绑定、backend 归属和验证要求。研究与前沿等需要跨任务延续的工作仍建模为持久状态；简历生成、演示文稿制作等明确产出结果的操作则作为面向用户的一次性功能。两者可以共享底层能力，但不会因为使用同一命令入口而混淆生命周期或权限边界。

### 简历模块由匿名模板定义，事实仍来自 Raw

简历的模块顺序、双语标题、类别映射和排序由模板 YAML 决定，DOCX 模板负责页面设置、样式和模块书签。工作区记录只用于维护措辞、核验状态和版本选择，不替代事实来源。正式渲染要求入选记录已经核验且具有可用 Raw 定位符，生成带日期、不可覆盖的 DOCX 文件和版本清单，以支持后续局部更新和版本比较。

公开仓库提供不含个人信息的完整学术简历模板、模板规范和示例记录。使用者应在自己的项目工作区复制模板，并在相应事实已经进入受管 Raw 后替换示例内容；个人履历、实际版本和活动项目工作区不进入公开仓库。

### 原生演示文稿制作保留修订与交付边界

演示文稿制作新增持久修订、草稿批准、锁定和确定性交付机制。布局受明确约束，交付副本继承来源敏感性规则，制作过程与摄入复核仍保持为两个独立功能。这样既能连续维护一份演示文稿，也能清楚区分正在修改的工作状态、已经批准的内容和面向交付的稳定文件。

### 提取、恢复与工程校验同步强化

PDF 提取和摄入恢复进一步收紧了失败状态与重试边界；工程图影响分析和验证覆盖更多契约一致性问题；隔离的私有知识维护工具继续遵守物理边界。上述改进保持 Raw 为事实层、Wiki 为可修订解释层、图为导航层，不扩大公开数据范围，也不改变冻结论文产物。

## 2026年9月26日工程更新补充（v0.9.1）

本次补丁增强公开发布门禁的可诊断性：当 GitHub Actions 中的发布回归出现未捕获异常时，测试脚本会把完整 traceback 输出为机器可读的错误注解。即使原始运行日志暂时不可访问，也能通过公开 Checks API 定位失败位置。该变更不修改知识模型、摄入契约或冻结论文产物；PDF 保留 v0.9.0 的已审阅工程补充，本段仅随默认在线阅读的 Markdown 版本发布。

## 2026年9月26日工程更新补充（v0.9.2）

本次补丁补全 GitHub Actions 固定校验环境中的 NumPy 与 Pillow，使公开功能注册表的运行时导入检查能在全新 Python 3.12 runner 上执行。该变更只修复 CI 依赖声明，不改变知识模型、摄入契约或冻结论文产物；PDF 继续保留 v0.9.0 的已审阅工程补充。

## 2026年9月26日工程更新补充（v0.9.3）

本次补丁继续补全 GitHub Actions 固定校验环境，加入可编辑演示文稿工具运行时导入所需的 python-pptx。该变更只修复 CI 依赖声明，不改变知识模型、摄入契约或冻结论文产物；PDF 继续保留 v0.9.0 的已审阅工程补充。

## 2026年9月26日工程更新补充（v0.10.0）

本次更新建立面向独立社区下游的公开协作边界。Ran-ASKS 继续作为由维护者主导的通用发行版与工程基线；其他团队可以在自己的仓库中发展不同领域的 Agent、ontology、connector、工作流和评测，并独立负责数据、发布、升级和用户支持。新的领域项目首先属于 Community Downstream，不会因为 fork 或使用 ASKS 名称就自动成为官方版本、ASKS-Compatible 或 Reference Distribution。目前 Ran-ASKS 未指定任何官方领域发行版。

当前 GitHub 仓库仍由受管源仓库生成。外部 pull request 是可审阅的贡献提案；接受后先回灌权威源并保留贡献者署名，通过完整发布校验后重新生成公开树。公开的治理、生态身份规则和社区下游登记模板记录这一流程，但不提前承诺插件 API 或 Extension Contract。该更新不改变 Raw、Wiki、图和冻结论文产物的证据边界。
