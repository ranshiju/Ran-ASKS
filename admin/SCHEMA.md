...
- 行政页面与其他通用文档统一使用 `三元组:` section，每条严格写成 `主体 | 谓词 | 客体`；不得再输出旧式 `行政主题`、`行政关系` section，也不得用自然语言箭头代替 pipe 三字段。keyword 类关系最多15个、其他导航关系最多8条；关系谓词限 `涉及`、`讨论`、`形成决策`、`依据`、`替代`、`汇报`、`发布者`、`负责人`、`承办部门`、`推动`、`申请事项`、`适用对象`。模板由 `graph_ingest.py prefill --page <wiki页>` 输出。
